...
string damage_msg(int damage, string type)
{
    ...
    case "抓伤":
        ...
        if (damage < 20)
            return "结果$p只被轻轻地蹭破了一点油皮。\n";
        else if (damage < 40)
            return "结果$p的$l上被划出一道细长的血痕。\n";
        else if (damage < 80)
            return "结果「唰」地在$p的$l上蹭掉一大片皮肉！\n";
        else if (damage < 120)
            return "结果「唰」地一声擦出一道血淋淋的伤口！\n";
        else if (damage < 160)
            return "结果「唰」地一声$p的$l上被撕下血淋淋的一片肉！\n";
        else
            return "结果只听见$n一声惨嚎，$p$l的皮肉尽裂，露出白森森的骨头！\n";
        break;
    case "割伤":
    case "劈伤":
    case "砍伤":
        if (damage < 20)
            ...
            return "结果「嗤」地一声砍出一道伤口！\n";
        else if (damage < 120)
            return "结果「嗤」地一声劈出一道血淋淋的伤口！\n";
        else if (damage < 160)
            return "结果「嗤」地一声劈出一道又长又深的伤口，溅得$N满脸鲜血！\n";
        else
            return "结果只听见$n一声惨嚎，$w已在$p$l砍出一道深及见骨的可怕伤口！！\n";
        break;
    case "刺伤":
        if (damage < 20)
    ...
    case "跌伤":
        if (damage < 20)
            return "结果$p就地一滚，没怎么伤着。\n";
        else if (damage < 40)
            return "结果「吧叽」一声，$p被摔了个屁墩。\n";
        else if (damage < 80)
            return "结果$p的$l着实地被摔了一下！\n";
        else if (damage < 120)
            return "结果$p的$l砸在地上，被摔得青紫！\n";
        else if (damage < 160)
            return "结果在$p的$l立时肿起一大块，鲜血里透着青紫！\n";
        else
            return "结果$p的$l被摔得皮开肉绽，骨头从肉里戳出来！！\n";
        break;
    case "鞭伤":
        if (damage < 20)
            return "结果只是在$p$l上擦了一道白印。\n";
        else if (damage < 40)
            return "结果在$p的$l上抽出一道血痕。\n";
        else if (damage < 80)
            return "结果「啪」地一声$p被抽了一道伤口！\n";
        else if (damage < 120)
            return "结果$p的$l上被抽了一道血淋淋的创口！\n";
        else if (damage < 160)
            return "结果「啪」地一声$p被$N的$w抽了一道从头到脚的血印！\n";
        else
            return "结果只听见「咔嚓」一声，$p的$l的骨头被抽断，$w被染得血红！！\n";
        break;
    case "咬伤":
        if (damage < 20)
            return "结果只是轻轻地蹭了一下$p的皮肉。\n";
        else if (damage < 40)
            return "结果在$p$l咬出一排牙印。\n";
        else if (damage < 80)
            return "结果$p被咬下一片肉来！\n";
        else if (damage < 120)
            return "结果$p连皮带肉被咬下一大块！\n";
        else if (damage < 160)
            return "结果在$p的身上咬下来血肉模糊的一大块$l肉！\n";
        else
    ...
    case "挫伤":
        if (damage < 10)
            ...
            return "结果这一下「砰」地一声击得$n连退了好几步，差一点摔倒！\n";
        else if (damage < 240)
...
varargs int skill_power(object ob, string skill, int usage)
{
    ...
    if (level < 1)
        return (int)(ob->query("combat_exp") / 20 * (jingli_bonus / 10));
    ...
    if (usage == SKILL_USAGE_ATTACK)
    {
        if ((ob->query_str() / 10) * (jingli_bonus / 10) > 300)
            ...
            return (power + ob->query("combat_exp")) / 30 * (ob->query_str() / 10) * (jingli_bonus / 10);
    }
    else
    {
        if ((ob->query_dex() / 10) * (jingli_bonus / 10) > 300)
            ...
            return (power + ob->query("combat_exp")) / 30 * (ob->query_dex() / 10) * (jingli_bonus / 10);
    }
}
...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
    ...
    if (victim->query("combat_exp") < 0)
        victim->set("combat_exp", 0);
    ...
    { // Does the victim dodge this hit?
        ...
        if (dp <= ap && (!userp(victim) || !userp(me)) && random(your["jingli"] * 100 / (your["max_jingli"] + 1) + your["int"]) > 50)
        {
            your["combat_exp"] += 1;
    ...
    else
    {
        ...
        if (victim->query_temp("weapon"))
        {
            ...
            if (!weapon && undefinedp(action["weapon"]))
                pp *= 2;
        }
        else
        {
        ...
        if (random(ap + pp) < pp)
        {
            ...
            if (pp <= ap && (!userp(victim) || !userp(me)) && random(your["jingli"] * 100 / (your["max_jingli"] + 1) + your["int"]) > 50)
            {
                your["combat_exp"] += 1;
        ...
        else
        {
            ...
            // equal skill, equal power
            if (!objectp(weapon))
                damage_bonus += ((int)me->query_skill(attack_skill) / 4 + (int)me->query_skill("force") / 2 + 1) / 10 * (damage_bonus / 10);
            else
            ...
            if (random(damage) > (int)victim->query_temp("apply/armor") && ((me->is_killing(victim->query("id"))) && ((!weapon) && !random(4) || weapon && !random(2)) || ((!weapon) && !random(7) || weapon && !random(4))))
            {
            ...
            if (!userp(me) || !userp(victim))
            {
                if ((ap <= dp) && !me->query_temp("yield") && !victim->query_temp("yield") && (random(my["jingli"] * 100 / my["max_jingli"] + my["int"]) > 30))
                {
                    my["combat_exp"] += 1;
                ...
                if (victim->query_temp("yield") && damage <= your["qi"] / 2 && random(my["combat_exp"]) >= random(your["combat_exp"]) && random(5) == 0)
    ...
    if (attack_type == TYPE_REGULAR && damage < 1 && victim->query_temp("guarding") && random(1 - (int)victim->query_temp("apply/speed")) < random((1 - (int)me->query_temp("apply/speed")) * 6))
    {
...
void fight(object me, object victim)
{
    ...
    if (victim->is_busy() || !living(victim))
    {
        ...
        if (me->is_fighting(victim) && victim->is_fighting(me))
            if ((!objectp(me->query_temp("weapon")) && sizeof(me->query_skill_prepare()) > 1) || (objectp(me->query_temp("weapon")) && (me->query_temp("weapon"))->query("skill_type") == "sword" && me->query_skill("pixie-jian", 1) >= 60 && me->query_skill_mapped("sword") == "pixie-jian" && me->query("gender") == "无性") || (me->query("double_attack") && !me->query_temp("weapon")))
            {
    ...
    else if (random((int)victim->query_dex() * 3) < (int)me->query_str() * 2 + (int)me->query_temp("apply/speed"))
    {
        me->set_temp("guarding", 0);
        if (!victim->is_fighting(me))
            victim->fight_ob(me);
...
void start_berserk(object me, object obj)
{
    ...
    if (shen > (int)me->query("quest_exp") && !wizardp(obj))
    {
...
void killer_reward(object killer, object victim)
{
    ...
    if (!environment(victim)->query("no_death"))
    {
        ...
        if (userp(victim))
        {
            ...
            if (killer->query("race") == "野兽" || killer->query("race") == "蛇类" || killer->query("race") == "游鱼" || killer->query("race") == "昆虫")
                mode = "咬";
            else if (killer->query("race") == "家畜")
                mode = "踩";
            else if (killer->query("race") == "飞禽")
                mode = "啄";
            else
                mode = "杀";
        ...
        if (userp(killer) && !killer->query_temp("free_rider") && killer->query("combat_exp") < victim->query("combat_exp") && killer->query("combat_exp") > victim->query("combat_exp") / 4)
